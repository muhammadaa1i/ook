...
const ProductCard: React.FC<ProductCardProps> = ({ slipper, onAddToCart, onViewDetails }) => {
  ...
  const imageUrls = useMemo(() => {
    ...
    if (urls.length === 0 && slipper.image_gallery && slipper.image_gallery.length > 0) {
      slipper.image_gallery.forEach(path => {
    ...
    if (urls.length === 0 && slipper.primary_image) {
      urls.push(getFullImageUrl(slipper.primary_image));
    }

    // Priority 4: Legacy fallback to single image field
    if (urls.length === 0 && slipper.image) {
      urls.push(getFullImageUrl(slipper.image));
    }

    if (!urls.length) urls.push("/placeholder-product.svg");
    return urls;
  }, [slipper.images, slipper.image_gallery, slipper.primary_image, slipper.image]);
  ...
  return (
    <div
      className={
        `relative flex flex-col bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-all duration-200 cursor-pointer border ${inCart && !isAdmin ? "border-blue-500 ring-1 ring-blue-400/40" : "border-gray-100"
        }`
      }
